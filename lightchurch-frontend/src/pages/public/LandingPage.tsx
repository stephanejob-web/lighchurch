...
const LandingPage: React.FC = () => {
    ...
    return (
        <Box sx={{ bgcolor: '#050505', color: 'white', minHeight: '100vh', overflowX: 'hidden' }}>
                ...
                <Container maxWidth="lg">
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Stack direction="row" spacing={4} sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center' }}>
                            <Typography sx={{ cursor: 'pointer', opacity: 0.7, fontSize: '0.95rem', '&:hover': { opacity: 1 } }} onClick={() => navigate('/map')}>Explorer</Typography>
                            <Typography sx={{ cursor: 'pointer', opacity: 0.7, fontSize: '0.95rem', '&:hover': { opacity: 1 } }}>Gérer ma communauté</Typography>
                            <Button variant="outlined" onClick={() => navigate('/login')} sx={{ borderColor: 'white', color: 'white', borderRadius: 50, textTransform: 'none', fontWeight: 500, px: 3, py: 1, '&:hover': { bgcolor: 'white', color: 'black' } }}>Rejoindre le réseau</Button>
                        </Stack>
            ...
            <Box sx={{ py: { xs: 8, md: 12 }, position: 'relative' }}><ParticleBackground /><Container maxWidth="md" sx={{ position: 'relative', zIndex: 1 }}><Box sx={{ p: { xs: 5, md: 8 }, borderRadius: 6, bgcolor: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.1)', textAlign: 'center' }}><Typography variant="h3" sx={{ fontWeight: 700, mb: 3, fontSize: { xs: '1.5rem', md: '2rem' } }}>Vous êtes responsable d'église ?</Typography><Typography sx={{ opacity: 0.5, mb: 5, maxWidth: 550, mx: 'auto' }}>Référencez gratuitement votre église sur LightChurch. Mettez à jour vos horaires, publiez vos événements, et soyez visible par des milliers de personnes.</Typography><Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="center"><Button variant="contained" onClick={() => navigate('/register')} sx={{ bgcolor: '#4285F4', borderRadius: 50, py: 1.5, px: 4, fontWeight: 500, textTransform: 'none', '&:hover': { bgcolor: '#3367D6' } }}>Référencer mon église gratuitement</Button><Button variant="outlined" onClick={() => navigate('/login')} sx={{ borderColor: 'rgba(255, 255, 255, 0.3)', color: 'white', borderRadius: 50, py: 1.5, px: 4, fontWeight: 500, textTransform: 'none', '&:hover': { borderColor: 'white' } }}>J'ai déjà un compte</Button></Stack></Box></Container></Box>
